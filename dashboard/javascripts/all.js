  ...
  */
  ...
  window.matchMedia = window.matchMedia || (function( doc, undefined ) {
    ...
    return function(q){
      ...
      div.innerHTML = "&shy;<style media=\"" + q + "\"> #mq-study-1 { width: 42px; }</style>";
  ...
  window.Foundation = {
    ...
    lib_methods : {
      ...
      // study for empty object or array
      empty : function (obj) {
...
window.Modernizr = (function( window, document, undefined ) {
    ...
    injectElementWithStyles = function( rule, callback, nodes, testnames ) {
      ...
      if ( parseInt(nodes, 10) ) {
          // In order not to give false positives we create a node for each study
      ...
      div.id = mod;
      // IE6 will false positive on some tests due to the style element inside the study div somehow interfering offsetHeight, so insert it into body or fakebody.
    ...
    // testProps is a generic CSS / DOM property study.

    // In testing support for a given CSS property, it's legit to study:
    //    `elem.style[styleName] !== undefined`
    // If the property is supported it will return an empty string,
    // if unsupported it will return undefined.

    // We'll take advantage of this quick study and skip setting a style
    ...
    /**
     * testDOMProps is a generic DOM property study; if a browser supports
    ...
    /*
     * The Modernizr.touch study only indicates if the browser supports
     ...
     *
     * We also study for Firefox 4 Multitouch Support.
     *
    ...
    // For speed, we don't study the legacy (and beta-only) indexedDB
    ...
    // WebOS unfortunately false positives on this study.
    tests['boxshadow'] = function() {
        return testPropsAll('boxShadow');
    };

    // FF3.0 will false positive on this study
    ...
    // Note, Android < 4 will pass this study, but can only animate
    ...
    // Normally, we could not study that directly and need to do a
    //   `('localStorage' in window) && ` study first because otherwise Firefox will
    ...
    // study page: paulirish.com/demo/inline-svg
    ...
    // This study is only for clip paths in SVG proper, not clip paths on HTML content
            ...
            for ( var i = 0, bool, inputElemType, defaultView, len = props.length; i < len; i++ ) {
                ...
                if ( bool ) {
                    ...
                    if ( /^range$/.test(inputElemType) && inputElem.style.WebkitAppearance !== undefined ) {
                      ...
                      // Interestingly, opera fails the earlier study, so it doesn't
                      //  even make it here.
    ...
    for ( var feature in tests ) {
        if ( hasOwnProp(tests, feature) ) {
            // run the study, throw the return value into the Modernizr,
     ...
     Modernizr.addTest = function ( feature, test ) {
       if ( typeof feature == 'object' ) {
         ...
         if ( Modernizr[feature] !== undefined ) {
           // we're going to quit if you're trying to overwrite an existing study
    ...
    /*>>hasevent*/
    // Modernizr.hasEvent() detects support for a given event, with an optional element to study on
    ...
    /*>>teststyles*/
    // Modernizr.testStyles() allows you to add custom styles to the document and study an element afterwards
